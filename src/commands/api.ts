...
import { getErrorForDomain, getErrorForPassword, getErrorForMachineName, userCancelOperation } from '../utils/ValidationsHandler'
...
export default class Api extends Command {
    ...
    protected description = 'Call a generic API on a specific CapRover machine. Use carefully only if you really know what you are doing!'
    ...
    protected options = (params?: IParams): IOption[] => [
        ...
        },
        {
            ...
            char: 'n',
            env: 'CAPROVER_NAME',
            message: params ? 'select the CapRover machine name you want to call API to' : 'CapRover machine name, to load/store credentials',
            type: 'list',
        ...
        CliHelper.get().getEnsureAuthenticationOption(params, async (machine: IMachine) => {
            ...
            try {
                ...
                StdOutUtil.printError(`\nSomething bad happened during calling API to ${StdOutUtil.getColoredMachineUrl(machine.baseUrl)}.\n${e.message || e}`, true)
            }
        }),
        {
            name: K.path,
            ...
            validate: (path: string) => path && path.startsWith('/') ? true : 'Please enter a valid path.'
        },
        {
            name: K.method,
            char: 'm',
            env: 'CAPROVER_API_METHOD',
            message: params ? 'select the API method you want to call' : `API method to call, one of: ${CliHelper.get().getApiMethodsDescription()}`,
            type: 'list',
            default: params && 'GET',
            choices: CliHelper.get().getApiMethodsAsOptions(),
            filter: (method: string) => !this.param(params, K.method) ? userCancelOperation(!method, true) || method : method.trim(),
            validate: (method: string) => method && Constants.API_METHODS.includes(method) ? true : `Please enter a valid method, one of: ${CliHelper.get().getApiMethodsDescription()}`
        },
        {
            name: K.data,
            char: 'd',
            env: 'CAPROVER_API_DATA',
            message: 'API data JSON string' + (params ? '' : ' (or also JSON object from config file), for "GET" method they are interpreted as querystring values to be appended to the path'),
            type: 'input',
            filter: data => {
                if (data && typeof data === 'string') try { return JSON.parse(data) } catch { }
                return data
            },
            validate: data => {
        ...
        },
        {
            ...
            filter: (out: string) => {
                if (!out) return 'false'
                out = out.trim() || 'false'
                if (out === 'true' || out === 'false' || isAbsolute(out)) return out
        ...
        },
        {
            ...
            default: true,
            hide: true,
            when: () => this.paramFrom(params, K.name) === ParamType.Question || this.paramFrom(params, K.path) === ParamType.Question || this.paramFrom(params, K.data) === ParamType.Question,
        ...
        try {
            ...
            if (out === 'true') {
                ...
                try {
                    ...
                    StdOutUtil.printWarning(`Error writing API response to file: "${out}".\n`)
                }
            }
        } catch (error) {
            StdOutUtil.printError(`\nSomething bad happened calling API ${StdOutUtil.getColoredMachineUrl(this.paramValue<string>(params, K.path)!)} at ${this.machine.name ? StdOutUtil.getColoredMachineName(this.machine.name) : StdOutUtil.getColoredMachineUrl(this.machine.baseUrl)}.`)
