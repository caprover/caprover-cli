...
export default class DeployTar extends Command {
    protected command = 'deploy-tar'

    protected usage =
        '[options]\n' +
        '       deploy -d\n' +
        '       deploy -c file\n' +
        '       deploy [-c file] [-n name] [-a app] [-t tarFile | -i image]\n' +
        '       deploy [-c file] -u url [-p password] [-n name] [-a app] [-t tarFile | -i image]\n' +
        ...
        '  Use one among --tarFile, --imageName'
    ...
    protected options = (params?: IParams): IOption[] => [
        ...
        },
        {
            ...
            message: 'directory to upload via tar file',
            type: 'input',
            default: params && './',
            when: !this.findParamValue(params, K.tar),
            validate: (tarDir: string) => getErrorForTarPath(path.join(process.cwd(), tarDir)),
        },
        {
            name: K.img,
            char: 'i',
                ...
                this.options = (params?: IParams) => [CliHelper.get().getEnsureAuthenticationOption(
                    ...
                    async (machine: IMachine) => {
                        ...
                        if (params) {
                            ...
                            if (possibleApp.deploySource.tarFilePath)
                                params[K.tar] = {
    ...
    protected validateDeploySource(params: IParams) {
        if (
            ...
            StdOutUtil.printError(
                'Only one of tarFile or imageName can be present in deploy.\n',
                true
            )
        }
        if (
