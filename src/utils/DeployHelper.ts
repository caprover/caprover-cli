...
export default class DeployHelper {
    ...
    async startDeploy(deployParams: IDeployParams): Promise<boolean> {
        ...
        if ((branchToPush ? 1 : 0) + (imageName ? 1 : 0) + (tarFilePath ? 1 : 0) !== 1) {
            StdOutUtil.printError('Can\'t deploy: only one of branch, tarFile or imageName can be present.\n', true)
            return false
        }
        ...
        const tarFileFullPath = tarFileNameToDeploy.startsWith('/') ? tarFileNameToDeploy : path.join(process.cwd(), tarFileNameToDeploy)
        ...
        StdOutUtil.printMessage(`Deploying ${StdOutUtil.getColoredAppName(appName)} to ${machineToDeploy.name ? StdOutUtil.getColoredMachineName(machineToDeploy.name) : StdOutUtil.getColoredMachineUrl(machineToDeploy.baseUrl)}...\n`)
        try {
            if (imageName) {
                await CliApiManager.get(machineToDeploy).uploadCaptainDefinitionContent(
                    appName,
    ...
    private gitArchiveFile(zipFileFullPath: string, branchToPush: string) {
        return new Promise<string>(function(resolve, reject) {
            if (fs.pathExistsSync(zipFileFullPath)) fs.removeSync(zipFileFullPath) // Removes the temporary file created
            ...
            exec(`git archive --format tar --output "${zipFileFullPath}" ${branchToPush}`, (err, stdout, stderr) => {
                ...
                exec(`git rev-parse ${branchToPush}`, (err, stdout, stderr) => {
                    ...
                    if (err || !/^[a-f0-9]{40}$/.test(gitHash)) {
                        StdOutUtil.printError(`Cannot find hash of last commit on branch "${branchToPush}": ${gitHash}\n${err}\n`)
                        if (fs.pathExistsSync(zipFileFullPath)) fs.removeSync(zipFileFullPath)
                        reject(new Error('rev-parse failed'))
                        return
                    }

                    StdOutUtil.printMessage(`Using last commit on "${branchToPush}": ${gitHash}\n`)
                    resolve(gitHash)
                })
            })
        })
    ...
    private async onLogRetrieved(data: IBuildLogs | undefined, machineToDeploy: IMachine, appName: string) {
        if (data) {
            ...
            if (firstLineNumberOfLogs > this.lastLineNumberPrinted) {
                if (firstLineNumberOfLogs < 0) { // This is the very first fetch, probably firstLineNumberOfLogs is around -50
        ...
        if (data && !data.isAppBuilding) {
            if (!data.isBuildFailed) {
                let appUrl = machineToDeploy.baseUrl.replace('https://', 'http://').replace('//captain.', `//${appName}.`)
                if (this.ssl) appUrl = appUrl.replace('http://', 'https://')
                StdOutUtil.printGreenMessage(`\nDeployed successfully ${StdOutUtil.getColoredAppName(appName)}`)
                StdOutUtil.printGreenMessage(`App is available at ${StdOutUtil.getColoredMachineUrl(appUrl)}\n`, true)
            } else {
