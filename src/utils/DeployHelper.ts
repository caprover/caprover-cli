...
export default class DeployHelper {
                    ...
                    exec(
                        ...
                        (err, stdout, stderr) => {
                            ...
                            StdOutUtil.printMessage(
                                `Pushing last commit on ${branchToPush}: ${gitHash}`
                            )
                            resolve(gitHash)
                        }
                    )
    ...
    async startDeploy() {
        ...
        if (
            ...
            StdOutUtil.printError(
                'Default deploy failed. imageName/branchToPush/tarFilePath cannot all be present.',
                true
            )
            return
        }
        ...
        StdOutUtil.printMessage(
            `Deploying ${appName} to ${machineToDeploy.name}`
        )
        ...
        try {
            if (imageName) {
                ...
                StdOutUtil.printMessage(
                    `Uploading the file to ${machineToDeploy.baseUrl}`
                )
    ...
    private async onLogRetrieved(
        ...
        if (data && !data.isAppBuilding) {
            if (!data.isBuildFailed) {
                const appUrl = self.deployParams
                    .captainMachine!.baseUrl.replace('https://', 'http://')
                    .replace('//captain.', '//' + appName + '.')
                StdOutUtil.printGreenMessage(
                    `\n\n\nDeployed successfully: ${appName}`
                )
                StdOutUtil.printMagentaMessage(
                    `App is available at ${appUrl}`,
                    true
                )
            } else {
                StdOutUtil.printError(
